"""SingGraph benchmark dataset and Lightning DataModule."""
...
class SingGraphDataset(Dataset):
    """Loads pre-aligned SingGraph vocals/instrumental features from index.json."""
    ...
    def __init__(
        ...
        *,
        ...
        if not self.entries:
            raise ValueError(
                f"No SingGraph entries found in {index_path} (split={split})"
            )
    ...
    def __getitem__(self, index: int):
        ...
        vocals = torch.load(
            entry["singgraph_vocals"], map_location="cpu", weights_only=True
        )
    ...
    def _check_shape(self, tensor: torch.Tensor, stem: str, name: str) -> None:
        if tensor.ndim != 3:
            raise ValueError(
                f"{name} rank {tensor.ndim} != 3 for {stem}: {tuple(tensor.shape)}"
            )
...
class SingGraphDataModule(L.LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int,
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        self.training_dataset = SingGraphDataset(
            index_path, split="train", **ds_kwargs
        )
        self.valid_dataset = SingGraphDataset(index_path, split="valid", **ds_kwargs)
        self.test_dataset = SingGraphDataset(index_path, split="test", **ds_kwargs)

        logging.info(
            "SingGraph splits: train=%d valid=%d test=%d",
