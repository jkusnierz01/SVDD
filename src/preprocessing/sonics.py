...
class SonicsPreprocessor(BaseProcessor):
    ...
    def preprocess(self):
        ...
        mert_processor = Wav2Vec2FeatureExtractor.from_pretrained(self.mert_model_name, trust_remote_code=True)
        mert_model = AutoModel.from_pretrained(self.mert_model_name, trust_remote_code=True).to(self.device)
        ...
        for batch_chunks, filenames in tqdm(dataloader, desc="Processing Batches"):
            ...
            try:
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    # --- A. Wav2Vec (Speech) ---
                    # Normalize & Prepare
                    inputs_w2v = wav2vec_processor(flat_input, sampling_rate=self.sample_rate, return_tensors="pt", padding=False)
                    ...
                    # --- B. MERT (Music) ---
                    inputs_mert = mert_processor(flat_input, sampling_rate=self.sample_rate, return_tensors="pt", padding=False)
                ...
                out_w2v = out_w2v.view(current_batch_size, num_chunks, seq_len, hidden_dim).float().cpu()
                out_mert = out_mert.view(current_batch_size, num_chunks, seq_len, hidden_dim).float().cpu()
